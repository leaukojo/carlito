...
## driveline retarder, and the drive-axle load read. Pure statics, no physics body.
##
## The two behavioural claims it exists to pin, because both are quiet failures: the retarder
## cannot lock a wheel (its torque is capped below what the road can answer at that contact patch,
## a property of the model rather than a clamp someone might tune away), and the §6 hierarchy
## (brake > peak drive > handbrake) still holds on every shipped truck spec with the retarder on.
...
## named by path: an orphan spec left behind by a refactor kept test_tractor green with the
## driveline flags dead in-game.
...
func test_the_warning_sits_above_the_gate_so_there_is_a_band_to_stop_in() -> void:
	# The contract's 'warn' is the low-pressure warning, not the cut-in. Collapse them onto one number
	# and the driver goes from a red bar to immobile in the same instant.
	...
	assert_float(TruckT.AIR_SPRING_BRAKE_BAR) \
		...
		.is_less(air.warn)
	# The spawn pressure is drivable but not full: the truck pulls away immediately and the bars
	# visibly charge afterwards.
...
func test_the_slip_cap_reads_a_SIGNED_road_speed_and_works_in_reverse() -> void:
	# BaseVehicle passes linear_velocity.dot(-basis.z), which is negative in reverse, and the cap's
	# signf() depends on that: a magnitude would leave the headroom saturated while reversing and
	# quietly disable the backstop.
	var dt := 1.0 / 60.0
	var r := 0.36
	var v := 20.0
...
func test_the_retarder_is_inert_at_a_standstill_however_the_wheels_are_spinning() -> void:
	# At rest the cap degenerates to a nonzero ceiling (signf(0) == 0). Not a hole: the demand is
	# faded to exactly 0 below RETARDER_CUTOUT_MS off the same road speed and retarder_torque takes
	# the minimum. Asserted because a ceiling looks like permission.
	var dt := 1.0 / 60.0
...
func test_the_retarder_can_never_skid_the_driven_axle() -> void:
	# The guarantee: however long the retarder is held on, it cannot drive the driven axle past
	...
	for spec in _truck_specs():
		...
		for v: float in [30.0, 22.0, 15.0, 8.0, 4.0, 2.0, -22.0, -8.0]:
			...
			var slip: float = (absf(v) - absf(omega) * gd.wheel_radius) / denom
			assert_float(slip) \
...
func test_the_slip_cap_stays_a_backstop_and_not_the_operating_point() -> void:
	# At the previous rating the axle settled near 0.004 slip, so the cap was unreachable and the
	# "cannot lock a wheel" guarantee was only exercised open-loop. Pinned the other way round: at the
	# slip target the road can make strictly more force than the retarder ever demands, so the
	# equilibrium provably sits below it. Monotone, so it needs no inverse of the grip curve.
	for spec in _truck_specs():
		var rear: int = _rear_count(spec)
		var gd := spec.ground_drive
		var demand_n: float = DrivetrainScript.retarder_rating(gd.brake_torque) / gd.wheel_radius
		...
		var grip_at_target: float = VehicleSpec.sample_curve(
				gd.grip_curve, DrivetrainScript.RETARDER_SLIP_TARGET) * gd.mu_long * rear_load_n
...
func test_every_truck_spec_keeps_the_force_hierarchy_with_the_retarder_added() -> void:
	# test_vehicle_catalog states the hierarchy over the generated specs and test_drivetrain pins it
	# on the sedan; the retarder is a new braking torque on the driven axle, so it is checked here on
	# the specs that carry it, and on the two hand-built units the generated sweep never sees.
	...
	for spec in specs:
		...
		var total_brake := gd.brake_torque * gd.wheel_positions.size()
		var total_handbrake := gd.handbrake_torque * 2.0
		var total_retarder := DrivetrainScript.retarder_rating(gd.brake_torque) * _rear_count(spec)
		...
		for p in gd.wheel_positions:
			if (p.z < 0.0 and gd.driven_front) or (p.z > 0.0 and gd.driven_rear):
				driven += 1
		var wheel_ceiling := spec.mass * 9.8 / maxi(gd.wheel_positions.size(), 1) \
				* gd.mu_long * gd.wheel_radius
		var transmissible := minf(max_drive, float(driven) * wheel_ceiling)
		assert_float(total_brake) \
			.override_failure_message("brake no longer beats transmissible drive") \
			.is_greater(transmissible)
		...
		# The retarder is an auxiliary brake: it must stay well under peak drive, and everything else
		# braking at once must come to less than the service brake, or the hierarchy has a back door.
...
func test_only_the_truck_declares_a_retarder() -> void:
	...
	for path in _spec_paths("res://src/vehicles"):
		...
		if spec.ground_drive == null:
			continue  # no running gear at all — the stronger form of "declares no retarder"
		checked += 1
		assert_bool(spec.ground_drive.retarder_equipped) \
...
#
# bus_up decides whether the second network is powered at all; is_inhibited is computed from
# chassis state and published on the body network. Nothing else in this project crosses a bus
# boundary.
...
# halves of one round trip: if they disagree, the number and the picture disagree.
...
func test_the_travel_stays_short_enough_not_to_clip_the_body() -> void:
	# Driving the first pass at -155 deg carried the arm through the cab instead of over it. There is
	# no cheap geometric predicate for "clips" without a physics query, so the shipped envelope is
	# pinned instead: an edit that doubles the travel back into the body has to be re-measured by
	# driving rather than reasoned about.
...
func test_a_full_hopper_is_a_payload_the_shipped_suspension_can_carry() -> void:
	# axle_load and engine_load report the mass coupling, which only works if the springs can hold the
	# loaded truck up: a payload past the suspension force cap would sit the chassis on its bump stops
	# and flatten the very signal it feeds.
	for spec in _truck_specs():
...
# both shipped specs with opposite values, off by default everywhere else, and a trailer COUPLED
# to the unit without it publishing honest zeros rather than a gap.
...
func test_the_north_american_unit_is_a_truck_family_BODY_variant() -> void:
	# A different tractor UNIT, not a different trailer, so it belongs to the V cycle and the E cycle
	# is untouched: one more truck-family entry through the same SemiTractor script.
...
func test_the_trailer_bus_flag_is_opposite_on_the_two_shipped_units() -> void:
	# The flag is only real on the spec the shipped scene loads, so both are read catalog -> scene ->
	# spec. Europe has the ISO 7638 data pair on pins 6 and 7; North America has no data pair at all
	# and puts trailer ABS on the power line instead.
	var by_variant := {}
	for variant in CatalogScript.variants_in_family("truck"):
...
func test_a_coupled_trailer_on_the_north_american_unit_publishes_honest_zeros() -> void:
	# TruckVehicle clears the whole bus every tick and SemiTractor only overwrites it behind
	# spec.trailer_bus_equipped, so this reproduces the publish step exactly for a unit without the
	# pair: real values are offered, the gate refuses them, and every trailer signal stays a real zero.
	for variant in CatalogScript.variants_in_family("truck"):
		var unit := _truck_scene(variant)
		var spec: VehicleSpec = unit.get("spec")
		unit.free()
		if spec.trailer_bus_equipped:
			continue
		var t := TruckT.new()
		# Values a coupled trailer really would produce, so a publish that ignored the gate is visible
		# rather than indistinguishable from a fresh struct.
...
func test_the_trailer_still_brakes_on_the_unit_with_no_bus() -> void:
	# The pneumatic lines are not the data pair: only the publishing goes dark, so the EBS11 blend the
	# trailer's own wheels brake with is computed the same way on both units.
...
func test_the_j2497_lamp_is_one_mirrored_bit_and_nothing_else() -> void:
	# SAE J2497 / PLC4TRUCKS carries trailer ABS on the power line as LAMP ON / LAMP OFF, so the whole
	# North American trailer protocol is one contract "in" bool with its own flavor: no range (that
	# would generate a bar), no counterpart "out" signal (nothing in the game may source it), and no
	# local timer anywhere, exactly like the DM1 lamps.
	var data := _contract()
...
func test_the_conventional_leaves_the_trailers_the_same_room_to_swing() -> void:
	...
	# chassis space — the same shape semi.tscn carries and test_trailer pins.
	var kingpin: Node3D = unit.get_node_or_null("FifthWheel/Kingpin")
	assert_object(kingpin) \
		.override_failure_message("conventional.tscn has no Kingpin marker").is_not_null()
	assert_vector((unit.get_node("FifthWheel") as Node3D).position) \
		.override_failure_message("the FifthWheel node is offset, so the marker is not the datum") \
		.is_equal(Vector3.ZERO)
	# The coupling plane may not move: every trailer is authored with its ground at y = -1.05 against
	# a plate top at y = 1.05, so a variant that raised or dropped it would float or bury them all.
	assert_float(kingpin.position.y) \
		.override_failure_message("the coupling plane is shared by every trailer - it cannot move") \
		.is_equal_approx(FifthWheelScript.KINGPIN_LOCAL.y, 1e-6)
