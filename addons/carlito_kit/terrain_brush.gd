...
## Terrain sculpt/paint brush, on the shared brush chassis: raise / lower /
## smooth / flatten on the heightmap image, and 8-channel painting across the
## terrain's two splat weight images.
##
## Editor-only: edits image content only, never the terrain's saved Texture2D
## or material. Live feedback remeshes touched chunks off the working image
## and previews paint via material_override on unowned chunks. Edits flush to
## PNG only on scene save (flush_all), never per stroke.

signal height_picked(y: float)  # eyedropper sample (world Y) -> the panel's fixed-height field

const BrushOps := preload("res://kit/helpers/brush_ops.gd")
const TerrainGen := preload("res://kit/terrain/terrain_gen.gd")

enum { OFF, RAISE, LOWER, SMOOTH, FLATTEN, RAMP, PAINT }  # index-matched to the panel's buttons

var mode := OFF
var channel := 0  # splat channel to paint, 0..7 (0..3 splatmap, 4..7 splatmap2)
## Square (Chebyshev) footprint instead of round, for pads that meet flush; the ramp ignores it.
var square := false
var flatten_fixed := false  # off = level to the drag's start height; on = level to flatten_height
var flatten_height := 0.0
var snap_step := 0.0  # quantize the flatten target to multiples of this many metres, 0 = off
## Snap the brush centre onto the road GridMap's cell lattice; falls back to a
## 12 m lattice at world origin with no GridMap.
var grid_snap := false
...
# Per-terrain edit session, kept until scene save (flush) or plugin exit.
...
# Active-stroke state. Paint edits both splat images, so snapshots come in
# pairs; sculpt only fills the first.
...
var _eff_mode := OFF  # the mode the running stroke applies, frozen at _stroke_begin

var _ramp_armed := false  # two-click ramp: start point stored between clicks
var _ramp_a := Vector3.ZERO
var _ramp_a_h := 0.0
var _pick_armed := false  # one-shot eyedropper: next click samples a height
...
## Esc cancels and is consumed; right-click cancels but is NOT consumed, so
## the editor still gets it for freelook.
...
## Shift smooths from any sculpt mode, Ctrl swaps raise/lower; frozen into
## _eff_mode at _stroke_begin so releasing a key mid-drag can't switch tools.
...
## Hugs the live working image, so it tracks sculpting in progress.
...
## Ray vs. heightfield, no physics (editor space isn't reliably populated).
...
## Snaps XZ onto GridMap cell centres (verified: a (12,3,12) GridMap centres
## cell 0 at local 6, not 0).
...
## Second ramp click: lay the whole A->B ramp as one edit.
...
## Flood the whole terrain with the selected channel; no strength/falloff.
...
## The normalized height a flatten stroke pulls toward, computed in world Y
## then converted to normalized.
...
## A world-circular brush on a non-square terrain is an ellipse in image space.
...
func _stroke_apply(center: Vector3) -> void:
	...
	if _eff_mode == PAINT:
		...
		# collision rebuild deferred to stroke end
...
## Undo/redo target: blit a saved region back into the working image.
...
## splat2 is created all-zero, sized like the splatmap, the first time a
## channel >= 4 is painted.
func _work_for(terrain: HeightmapTerrain, kind: String) -> Image:
	var id := terrain.get_instance_id()
	if not _sessions.has(id):
		# bound callable kept so teardown can disconnect the exact same one
...
## The terrain replaced a source image (Generate / Auto-splat / Conform /
## inspector edit): drop our working copy so the next touch decodes the new one.
...
## Live paint without touching the saved material: throwaway ImageTextures
## feed a duplicate material as material_override on the unowned chunks —
## writing them into the saved material would embed the raw images on save.
...
## Called after every paint sample and remesh, since _build_chunk recreates
## chunks with material_override = terrain.material.
...
## Tripwire + self-heal: the saved material must never hold a pathless
## (transient) splatmap texture.
...
## Matches HeightmapTerrain's height_at mapping.
